...
Dataset file2array(char* filename, int classNum, int itemNum){
	...
	double weights[] = {1.0, 1.5, 2.0, 2.5, 3.0};
	vector<double> weightClass;
	weightClass.assign(weights, weights+5);
	vector< vector< vector<double> > > datasetValues;
	for(int i = 0 ; i < classNum; i++) {
		...
		for(int j = 0; j<itemNum; j++) {
			...
			valueClass.push_back(input);
		}
		class1.push_back(valueClass);
		class1.push_back(weightClass);
		datasetValues.push_back(class1);
	}
...
int main() {
	...
	if( i == 1) {
		...
		data = file2array(PROFIT_FILE, 10000000, 5);
	}
