...
Dataset readFile(string fileName_y, double weights[], int size_weights, string fileName_cnt);
...
Dataset readFile(string fileName_y, double weights[], int size_weights, string fileName_cnt){
    ...
    getline(fin_y, pred_prob_all); // 表头
    getline(fin_cnt, group_cnt);
    while(getline(fin_y, pred_prob_all)&&getline(fin_cnt, group_cnt)){ // 数据
        ...
        for(int i = 0; i < size_weights; i++) {
            weightClass[i] = weightClass[i] * cnt;
        }
...
int main(){
    ...
    string fileName_y = "df_csg_s_sum_correct_limit_min.txt";
    string fileName_cnt = "df_csg_cnt_sum_correct.txt";
    string fileName_out = "df_csg_result_sum_correct_limit_min.txt";
    
    double weights[] = {0.2, 0.5, 1.0, 1.5, 1.8};
    double cac = 1.0;
    ...
    Dataset ds = readFile(fileName_y, weights, sizeof(weights)/sizeof(weights[0]), fileName_cnt);
