...
if __name__ == '__main__':
    ...
    if config.conf['dist_conf']['multi_proc']['is_multi'] == 1:
        ...
        dml = dist_multi_LP(config.conf['dist_conf']['base']['cols_score'], config.conf['dist_conf']['base']['cols_score_dist'], config.conf['dist_conf']['base']['item'], config.conf['dist_conf']['base']['ratio_item'], 
        ...
        df_res = dist.LP(df_rule_score, 
                         config.conf['dist_conf']['base']['cols_score'], config.conf['dist_conf']['base']['cols_score_dist'], config.conf['dist_conf']['base']['item'], config.conf['dist_conf']['base']['ratio_item'], 
                         config.conf['dist_conf']['base']['coef_flow_shrink'], 
                         config.conf['dist_conf']['base']['cols_rep'], 
    ...
    utils.statistics(df_res, config.date, config.conf['stat_conf']['role'], config.conf['stat_conf']['exp_mode'],
                     config.conf['dist_conf']['base']['index_exp_group'], config.conf['stat_conf']['item_name'], config.conf['dist_conf']['base']['item'],
