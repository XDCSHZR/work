...
def md5_hash(seed):
    ...
    hash by md5
    '''
    seed_md5 = hashlib.md5(seed.encode('utf-8')).hexdigest()
    seed_md5_num = int(seed_md5, 16)
...
def statistics(df, date, role, exp_mode,
               ...
               para_chat={'is_chat': 1,
                           'title': '',
                           'url': 'https://'}, 
               item_none_name='无效组', item_none_code=666):
    message = '\n数据日期：{}'.format(date) + \
              '\n角色：{}'.format(role) + \
              '\n实验类型：{}'.format(exp_mode) + \
              '\n实验组人群分组索引：{}'.format(list_index_exp_group) + \
              '\n实验组{}分组：{}'.format(item_name, list_item+[item_none_name]) + \
              '\n实验组{}分组线上编码：{}'.format(item_name, list_code_item+[item_none_code]) + \
              '\n实验组总流量：{}'.format(df.shape[0]) + \
              '\n实验组{}分组流量占比（预期）：{}'.format(item_name, list_ratio_item+[0.0]) + \
              '\n实验组{}分组流量缩放比例：{}'.format(item_name, coef_flow_shrink) + \
              '\n实验组{}分组流量（实际）：{}'.format(item_name, [df[df['xxx_code']==x].shape[0] for x in list_code_item+[item_none_code]]) + \
              '\n实验组{}分组流量占比（实际）：{}'.format(item_name, [round(df[df['xxx_code']==x].shape[0]/df.shape[0], 3)
                                                                 for x in list_code_item+[item_none_code]])
    print(message)
    if para_chat['is_chat'] == 1:
