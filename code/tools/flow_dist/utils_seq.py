...
def statistics(df, date, role, exp_mode,
               list_index_exp_group, item_name, list_item,
    ...
    message = '\n数据日期：{}'.format(date) + \
              ...
              '\n实验组{}分组流量（实际）：{}'.format(item_name, [df[df['xxx_order'].str.startswith(x)].shape[0] for x in list_item]) + \
              '\n实验组{}分组流量占比（实际）：{}'.format(item_name, [round(df[df['xxx_order'].str.startswith(x)].shape[0]/df.shape[0], 3)
                                                                 for x in list_item])
    print(message)
    if para_chat['is_chat'] == 1:
