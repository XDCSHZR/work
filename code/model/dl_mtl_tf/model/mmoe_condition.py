...
from utils.feature_column import split_features_condition
...
class MMOE(Model):
    def __init__(self, dnn_feature_columns,
        ...
        self.orderDict_features_dense_condition, self.orderDict_features_sparse, self.orderDict_feature_dense = split_features_condition(dnn_feature_columns)
    ...
    def call(self, inputs, training=None):
        inputs_dense_condition, inputs_sparse, input_dense = inputs[:, :len(self.orderDict_features_dense_condition)], inputs[:, len(self.orderDict_features_dense_condition):(len(self.orderDict_features_dense_condition)+len(self.orderDict_features_sparse))], inputs[:, (len(self.orderDict_features_dense_condition)+len(self.orderDict_features_sparse)):]
        ...
        task_outputs = []
        for i in range(len(self.task_names)):
            tower_output = self.tower_layers[i](tf.concat([tf.cast(mmoe_outputs[i], tf.float32), tf.cast(inputs_dense_condition, tf.float32)], axis=-1))
