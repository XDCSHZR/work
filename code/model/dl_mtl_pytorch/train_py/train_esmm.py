...
def process(df, mode='multi-label'):
    df.pop('uid')
    y = df['label']
    x = df[df.columns[:-1]]
    categorical_columns = ['xxx']
    ...
    continous_columns = [i for i in x.columns if i not in categorical_columns]
    x = pd.concat([x[categorical_columns], x[continous_columns]], axis=1)
    if mode == 'multi-label':  # multi-label or multi-class
        df['label_1'] = df['label'].replace(2, 1)  # label > 0 => 1
        df['label_2'] = df['label'].replace(1, 0)  # label < 2 => 0
        df['label_2'] = df['label_2'].replace(2, 1)
        # print(df['label_1'].value_counts(), df['label_2'].value_counts())
...
def get_columns(file_path):
    df = pd.read_csv(config.Data['trainFilePath'], sep='\t', iterator=True)
    tmp = df.get_chunk(1)
    tmp.pop('uid')
    x = tmp[tmp.columns[:-1]]
    categorical_columns = ['xxx']
...
def train(train_loader, test_loader, model, optimizer, scheduler, criterion, epochs, data_test, mode):
    for epoch in range(epochs):
        ...
        for i, (x, y) in enumerate(train_loader):
            ...
            pctr = torch.sigmoid(output[:, 0])
            pcvr = torch.sigmoid(output[:, 1])
            pctcvr = torch.mul(pctr, pcvr)
            # print(pctr.shape, pctcvr.shape)
            loss = criterion[1](pctcvr, y[:, 1]) + criterion[0](output[:, 0], y[:, 0])
            loss.backward()
            optimizer.step()
            if i % 100 == 0:
                logger.info('Epoch: [{}/{}], Step: [{}/{}], Lr: {:.6f}, Loss: {:.6f}'.format(
                    epoch + 1, epochs, i + 1, len(train_loader), optimizer.param_groups[0]['lr'], loss.item()))
            if i % 5000 == 0:
...
def val(test_loader, model, criterion, data_test, mode):
    ...
    with torch.no_grad():
        for i, (x, y) in enumerate(test_loader):
            ...
            pcvr = torch.sigmoid(output[:, 1])
            pctcvr = torch.mul(pctr, pcvr)
            output1 = torch.cat([pctr.unsqueeze(dim=1), pctcvr.unsqueeze(dim=1)], dim=1)
            test_loss = test_loss + criterion[0](output[:,0], y[:,0]).item() + criterion[1](pctcvr, y[:,1]).item()
            if i == 0:
    ...
    logger.info('Test set: Average loss: {:.4f}, {}'.format(test_loss, str_auc))
    ...
    if test_auc[-1] > best_auc:
        ...
        torch.save(model.state_dict(), './modelSaved/model.pth')
        model1 = torch.jit.script(model)
        torch.jit.save(model1, './modelSaved/esmm.pt')
        logger.info('Save model with loss: {:.4f}, {}'.format(test_loss, str_auc))


if __name__ == '__main__':
    # get configuration
    __CONF_PATH = './config/esmm.yaml'
    config = util.get_config(__CONF_PATH)
    util.seed_everything(config.Seed)

    logger.info("Model: ESMM")
    ...
    model = ESMM(config, feature_columns).to(device)
    ...
    if config.Model['loss'] == 'focalloss':
        ...
        criterion = (nn.BCEWithLogitsLoss().to(device), nn.BCELoss().to(device))
