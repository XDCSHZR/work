...
def val(test_loader, model, mode, task_num, epoch):
    ...
    if mode == 'multi-label':
        ...
        with torch.no_grad():
            for i, (x, y) in enumerate(test_loader):
                ...
                test_loss_list += np.array([i.item() for i in model.loss(output, y)])
    ...
    if epoch % 1 == 0:
        torch.save(model.state_dict(), './modelSaved/mfh_v1_0_epoch{}.pth'.format(str(epoch+1)))
        model1 = torch.jit.script(model)
        torch.jit.save(model1, './modelSaved/mfh_v1_0_epoch{}.pt'.format(str(epoch+1)))
...
if __name__ == '__main__':
    ...
    model = MFH(config, feature_columns).to(device)
