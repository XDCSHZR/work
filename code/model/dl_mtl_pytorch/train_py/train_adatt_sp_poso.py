...
def val(test_loader, model, criterion, mode, task_num):
    ...
    if test_auc[-1] > best_auc: # and test_auc[0] > 0.81 and test_auc[1] > 0.83:
        best_auc = test_auc[-1]
        torch.save(model.state_dict(), './modelSaved/adatt_sp_poso_v1_0.pth')
        model1 = torch.jit.script(model)
        torch.jit.save(model1, './modelSaved/adatt_sp_poso_v1_0.pt')
        logger.info('Save model with loss: {:.4f}, {}, {}'.format(test_loss, str_loss, str_auc))


if __name__ == '__main__':
    # get configuration
    __CONF_PATH = './config/adatt_sp_poso.yaml'
    ...
    model = AdaTTSpPoso(config, feature_columns).to(device)
