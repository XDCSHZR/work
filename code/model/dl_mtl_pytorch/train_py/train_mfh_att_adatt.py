...
from model.mfh_att_adatt import *
        ...
        if epoch < stage[-1]:
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                if i % 100 == 0:
                    ...
                    # torch.jit.save(model1, './modelSaved/mfh_adatt_v1_0_epoch{}.pt'.format(str(epoch+1)))
                    # print('model saved')
...
def val(test_loader, model, mode, task_num, epoch):
    ...
    if epoch % 1 == 0:
        torch.save(model.state_dict(), './modelSaved/mfh_adatt_v1_0_epoch{}.pth'.format(str(epoch+1)))
        model1 = torch.jit.script(model)
        torch.jit.save(model1, './modelSaved/mfh_adatt_v1_0_epoch{}.pt'.format(str(epoch+1)))
...
if __name__ == '__main__':
    # get configuration
    __CONF_PATH = './config/mfh_adatt.yaml'
    ...
    model = MFHATTAdaTT(config, feature_columns).to(device)
