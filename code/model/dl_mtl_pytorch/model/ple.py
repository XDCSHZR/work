...
class CGC(nn.Module):
    def __init__(self, specific_input, specific_output, shared_input, shared_output, is_last, conf):
        ...
        self.specific_expert = nn.ModuleList([nn.ModuleList([MLPExpert(specific_input, specific_output, self.specific_hidden_units, self.dropout, self.use_bn) for _ in range(self.num_specific_expert)]) for _ in range(self.num_tasks)])  # num_tasks * specific expert
        self.shared_expert = nn.ModuleList([MLPExpert(shared_input, shared_output, self.shared_hidden_units, self.dropout, self.use_bn) for _ in range(self.num_shared_expert)]) # 1 * shared expert
        ...
        if not self.is_last:
            ...
            self.gate = nn.ModuleList([nn.Linear(self.specific_input, n_specific) for _ in range(self.num_tasks)] + [nn.Linear(self.shared_input, n_shared)])
    ...
    def forward(self, x, x_shared):  # 只考虑输出就行
        ...
        for i, expert_list in enumerate(self.specific_expert):
            ...
            for k, expert_sh in enumerate(self.shared_expert):
                tmp_shared.append(expert_sh(x[-1]) * gate[i][:, k + self.num_specific_expert].unsqueeze(1).repeat(1, self.shared_output))
            ...
            specific_expert_output.append(torch.cat([torch.stack(tmp_specific, dim=1).reshape(-1, self.specific_shape), torch.stack(tmp_shared, dim=1).reshape(-1, self.shared_shape)], dim=1))
        ...
        if self.is_last:
            ...
            for i, expert_list1 in enumerate(self.specific_expert):
                for j, expert1 in enumerate(expert_list1):
                    tmp_specific.append(expert1(x[i]) * gate[-1][:, i*self.num_specific_expert + j].unsqueeze(1).repeat(1, self.specific_output))

            tmp_shared = []
            for k, expert_sh1 in enumerate(self.shared_expert):
                tmp_shared.append(expert_sh1(x[-1]) * gate[-1][:, self.num_tasks*self.num_specific_expert + k].unsqueeze(1).repeat(1, self.shared_output))

            # Flatten for concatenation
            # torch.stack(tmp_specific, dim=1).reshape(-1, self.specific_shape * self.num_tasks)
            # torch.stack(tmp_shared, dim=1).reshape(-1, self.shared_shape)
            shared_expert_output.append(torch.cat([torch.stack(tmp_specific, dim=1).reshape(-1, self.specific_shape * self.num_tasks), torch.stack(tmp_shared, dim=1).reshape(-1, self.shared_shape)], dim=1))

            return torch.stack(specific_expert_output, dim=2), shared_expert_output[0]


class MLPPLE(nn.Module):
    def __init__(self, config, feature_columns):
        super(MLPPLE, self).__init__()
        conf = config.Model
        self.num_tasks = conf['num_tasks']
        ...
        if self.num_extraction_module == 1:
            self.extraction_module = nn.ModuleList([CGC(self.num_feature, self.specific_units, self.num_feature, self.shared_units, is_last=True, conf=conf)])
        elif self.num_extraction_module == 2:
            self.cgc_input = [CGC(self.num_feature, self.specific_units, self.num_feature, self.shared_units, is_last=False, conf=conf)]
            self.cgc_out = [CGC(self.specific_units * self.num_specific_expert + self.shared_units * self.num_shared_expert, self.specific_units, self.specific_units * self.num_specific_expert * self.num_tasks + self.shared_units * self.num_shared_expert, self.shared_units, is_last=True, conf=conf)]
            self.extraction_module = nn.ModuleList(self.cgc_input + self.cgc_out)
        else:
            self.cgc_input = [CGC(self.num_feature, self.specific_units, self.num_feature, self.shared_units, is_last=False, conf=conf)]
            self.cgc_mid = [CGC(self.specific_units * self.num_specific_expert + self.shared_units * self.num_shared_expert, self.specific_units, self.specific_units * self.num_specific_expert * self.num_tasks + self.shared_units * self.num_shared_expert, self.shared_units, is_last=False, conf=conf)] * (self.num_extraction_module - 2)
            self.cgc_out = [CGC(self.specific_units * self.num_specific_expert + self.shared_units * self.num_shared_expert, self.specific_units, self.specific_units * self.num_specific_expert * self.num_tasks + self.shared_units * self.num_shared_expert, self.shared_units, is_last=True, conf=conf)]
            self.extraction_module = nn.ModuleList(self.cgc_input + self.cgc_mid + self.cgc_out)

        self.towers = nn.ModuleList([MLPTower(self.specific_units * self.num_specific_expert + self.shared_units * self.num_shared_expert, 1, self.tower_hidden_units, self.dropout, self.use_bn) for _ in range(self.num_tasks)])
