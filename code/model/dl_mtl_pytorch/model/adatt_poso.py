...
from layer.poso import GateNet
from dataclasses import dataclass, field
from math import sqrt
from typing import List, Optional, Union


class AdaTTSpPoso(nn.Module):
    """
    ...
    def __init__(self, config, feature_columns):
        super(AdaTTSpPoso, self).__init__()
        ...
        self.poso_hidden_units = conf['poso_hidden_units']

        self.dense_feature_columns, self.sparse_feature_columns, self.condition_columns, self.personal_columns = feature_columns
        self.sparse_feature_columns_len = len(self.sparse_feature_columns)
        self.dense_feature_columns_len = len(self.dense_feature_columns)
        self.condition_feature_columns_len = len(self.condition_columns)
        self.poso_feature_columns_len = len(self.personal_columns)
        ...
        self.towers_head = nn.ModuleList([MLPTower(self.tower_out_units, 1, [], self.dropout, self.use_bn) for _ in
                                     range(self.num_tasks)])
        # self.poso = GateNet(poso_feature_columns_len, self.poso_hidden_units, self.total_experts_per_layer, self.dropout)
        self.poso_gate = GateNet(self.poso_feature_columns_len, self.poso_hidden_units, self.num_tasks, self.dropout)
        
        
    def forward(self, x):
        condition_inputs, sparse_inputs, dense_inputs, poso_inputs = x[:, :self.condition_feature_columns_len], x[:,self.condition_feature_columns_len:(self.condition_feature_columns_len + self.sparse_feature_columns_len)], x[:,(self.condition_feature_columns_len + self.sparse_feature_columns_len):(self.condition_feature_columns_len + self.sparse_feature_columns_len + self.dense_feature_columns_len)], x[:, (self.condition_feature_columns_len + self.sparse_feature_columns_len + self.dense_feature_columns_len):]
        ...
        for layer_i, (self_experts_layer_i, self_gate_weights_layer_i) in enumerate(zip(self.experts, self.gate_weights)):
            ...
            # print(self_experts_layer_i, self_gate_weights_layer_i)
        ...
        poso_gate = self.poso_gate(poso_inputs)
        # print(poso_gate.shape)
        # print(inputs.shape)
        
        tower = [t(torch.cat([inputs[:, i, :]*poso_gate[:, i].unsqueeze(1), condition_inputs], dim=1)) for i, t in enumerate(self.towers)]
...
class AdaTTWSharedExpsPoso(nn.Module):
    """
    paper title: "AdaTT: Adaptive Task-to-Task Fusion Network for Multitask Learning in Recommendations"
    paper link: https://doi.org/10.1145/3580305.3599769
    Call Args:
    ...
    def __init__(self, config, feature_columns):
        super(AdaTTWSharedExpsPoso, self).__init__()
